...
static int arduino_run_command(unsigned char cmd, unsigned int *val) {
	...
	// send cmd
	result = mi2c_i2c_write(ARDUINO, buff, 1);
...
static int itg3200_init(void) {
	...
	udelay(5);
	mi2c_i2c_write_reg(ITG3200, ITG3200_REG_SAMPLE_RATE_DIV, ITG3200_SMPLRT_DIV); // set sample rate
	udelay(5);
	mi2c_i2c_write_reg(ITG3200, ITG3200_REG_LP_FULL_SCALE, 0x18 + ITG3200_DLPF_CFG); //low pass filter configuration
	udelay(5);
